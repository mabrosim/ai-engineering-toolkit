...
AET runs a four-step workflow through a coordinated set of agents:

1. **Discover** — maps your project structure, file types, and theme clusters
2. **Extract** — mines coding rules, naming conventions, and patterns from your source
3. **Generate** — writes instruction files in `.instructions.md` format
4. **Validate** — checks output for NDA-safety and public-release readiness
...
your code
 ...
 Discover    ── maps project structure and file themes
    │
 Extract     ── mines coding rules and naming patterns
    │
 Generate    ── writes .instructions.md files
    │
 Validate    ── NDA-safety and public-release gate
    │
    ▼
.instructions.md
```
...
- **On-demand skills**: add the `skills/` path to `chat.skillsFilesLocations` in VS Code settings
